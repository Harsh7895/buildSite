  ...
  import MenuOptions from './menu-options'
    ...
    const Sidebar = async({id , type}:Props) => {
      ...
      const isWhiteLablededAgency = user.Agency.whiteLabel
      if(!details) return

      let sideBarLogo = user.Agency.agencyLogo || "/assets/plura-logo.svg"

      if(!isWhiteLablededAgency){
        if(type === 'subaccount'){
      ...
      return (
      <>
        <MenuOptions defaultOpen={true} details={details} id={id} sidebarLogo={sideBarLogo} sidebarOpt={sidebarOpt}
        subAccounts={subaccounts} user={user}/>
        <MenuOptions details={details} id={id} sidebarLogo={sideBarLogo} sidebarOpt={sidebarOpt}
        subAccounts={subaccounts} user={user}/>
      </>
      )
    }
